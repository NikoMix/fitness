...
### 7. The cap the user sets is not the cap that is enforced

`ReminderRefreshService.ReadPreferences` reads a user-settable `DailyCap` (line 121) and passes it to
the planner, but `LocalNotificationScheduler.CanScheduleMore` compares against the hard constant
`MaxNonCriticalNotificationsPerLocalDay = 4` (line 239) and ignores the preference. Lowering the cap
does not lower it.
...
*Gaps.* R3 unmet and internally inconsistent: the default cap is 4, not 3, and worse, the two halves of the system disagree. ReminderRefreshService.ReadPreferences reads a user-settable 'DailyCap' preference (line 121) and passes it to the planner, but LocalNotificationScheduler.CanScheduleMore compares against the hard constant MaxNonCriticalNotificationsPerLocalDay = 4 (line 239) and ignores the preference entirely, so lowering the cap does not lower it. R2 unmet: only Workout, Meal and Hydration have toggles in NotificationSettingsPage, Meal is read by nothing, and the DailyCheckIn and StreakProtection categories the planner actually honours have no UI at all. R4 unmet: suppression reasons are computed and returned, then discarded - StreaksPageViewModel.cs:222 counts them and shows a single sentence; nothing is recorded to diagnostics. R5 and AC3 unmet: there is no Reduce frequency action and no Disable all action on the settings page.
