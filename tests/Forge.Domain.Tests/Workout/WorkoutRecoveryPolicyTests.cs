...
public sealed class WorkoutRecoveryPolicyTests
{
    ...
    public void Classify_returns_resume_for_recent_unfinished_session()
    {
        ...
        var session = new WorkoutSession { StartedUtc = now.AddHours(-2), CompletedUtc = null };
    ...
    {
        var now = new DateTimeOffset(2026, 8, 20, 12, 0, 0, TimeSpan.Zero);
        var session = new WorkoutSession { StartedUtc = now.AddHours(-13), CompletedUtc = null };
    ...
    {
        var now = new DateTimeOffset(2026, 8, 20, 12, 0, 0, TimeSpan.Zero);
        var session = new WorkoutSession { StartedUtc = now.AddHours(-13), CompletedUtc = now.AddHours(-1) };
